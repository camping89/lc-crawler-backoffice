...
namespace LC.Crawler.BackOffice.Medias
{
    ...
    public class MediasAppService : ApplicationService, IMediasAppService
    {
        ...
        {
            var totalCount = await _mediaRepository.GetCountAsync(input.FilterText, input.Name, input.ContentType, input.Url);
            var items = await _mediaRepository.GetListAsync(input.FilterText, input.Name, input.ContentType, input.Url, input.Sorting, input.MaxResultCount, input.SkipCount);
        ...
        public virtual async Task<MediaDto> CreateAsync(MediaCreateDto input)
        {
            ...
            input.Name, input.ContentType, input.Url
            );
