...
public class ArticleManangerAladin : DomainService
{
    ...
    public ArticleManangerAladin(IArticleAladinRepository articleAladinRepository,
        ICategoryAladinRepository categoryAladinRepository, IMediaAladinRepository mediaAladinRepository,
    ...
    public async Task ProcessingDataAsync(List<ArticlePayload> articles)
    {
        ...
        foreach (var rawArticles in articleGroup)
        {
            try
            {
                ...
                var articleEntity =
                    await _articleAladinRepository.FirstOrDefaultAsync(x => x.Title.Equals(article.Title));
    ...
    private async Task CheckFormatEntity(Article articleEntity)
    {
        ...
        catch (Exception e)
        {
            if (e.GetType() == typeof(FormatException))
            {
                ...
                await _articleAladinRepository.DeleteAsync(articleEntity.Id);
            }
        }
    }
    ...
    {
        var articles = await _articleAladinRepository.GetListAsync();
        var categories = await _categoryAladinRepository.GetListAsync(_ => _.CategoryType == CategoryType.Article);
    ...
    public async Task<List<string>> GetErrorEncodeData()
    {
        ...
        var articleIds = (await _articleAladinRepository.GetQueryableAsync()).Select(x=>x.Id).ToList();
        foreach (var id in articleIds)
        {
            try
            {
                var article = await _articleAladinRepository.GetAsync(id);
                ...
                foreach (var mediaId in article.Medias.Select(_ => _.MediaId).ToList())
                {
                    try
                    {
                        var media = await _mediaAladinRepository.GetAsync(mediaId);
                    }
                    catch (Exception e)
                    {
