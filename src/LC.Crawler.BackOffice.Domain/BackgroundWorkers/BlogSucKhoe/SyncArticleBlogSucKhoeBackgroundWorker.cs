...
public class SyncArticleBlogSucKhoeBackgroundWorker : HangfireBackgroundWorkerBase
{
    ...
    private readonly MediaManagerBlogSucKhoe _mediaManagerBlogSucKhoe;
    private readonly IDataSourceRepository _dataSourceRepository;
    private string BASEURL = string.Empty;
    
    public SyncArticleBlogSucKhoeBackgroundWorker(IArticleBlogSucKhoeRepository articleBlogSucKhoeRepository, IMediaBlogSucKhoeRepository mediaBlogSucKhoeRepository, MediaManagerBlogSucKhoe mediaManagerBlogSucKhoe, IDataSourceRepository dataSourceRepository, ICategoryBlogSucKhoeRepository categoryBlogSucKhoeRepository)
    {
        _articleBlogSucKhoeRepository = articleBlogSucKhoeRepository;
        _mediaBlogSucKhoeRepository = mediaBlogSucKhoeRepository;
        _mediaManagerBlogSucKhoe = mediaManagerBlogSucKhoe;
    ...
    public override async Task DoWorkAsync()
    {
        ...
        var articles = await _articleBlogSucKhoeRepository.GetListWithNavigationPropertiesAsync();
    ...
    private async Task PostToWPAsync(ArticleWithNavigationProperties articleNav)
    {
        ...
        if (articleNav.Categories != null)
        {
            ...
            if (result != null)
            {
                ...
                await _articleBlogSucKhoeRepository.UpdateAsync(articleNav.Article, true);
            }
        }
    }
    ...
    private async Task<List<MediaItem>> PostMediasAsync(Article article)
    {
        if (article is { Medias: { } })
        {
            ...
            var medias = await _mediaBlogSucKhoeRepository.GetListAsync(x =>mediaIds.Contains(x.Id));
            //pass the Wordpress REST API base address as string
            var client =await InitClient();
            ...
            if (medias != null)
            {
                ...
                foreach (var media in medias)
                {
                    try
                    {
                        var stream = await _mediaManagerBlogSucKhoe.GetFileStream(media.Name);
