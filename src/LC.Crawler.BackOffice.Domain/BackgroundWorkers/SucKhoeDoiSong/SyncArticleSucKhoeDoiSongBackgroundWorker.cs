...
public class SyncArticleSucKhoeDoiSongBackgroundWorker : HangfireBackgroundWorkerBase
{
    ...
    private readonly MediaManagerSucKhoeDoiSong _mediaManagerSucKhoeDoiSong;
    private readonly IDataSourceRepository _dataSourceRepository;
    private string BASEURL = string.Empty;
    public SyncArticleSucKhoeDoiSongBackgroundWorker(IArticleSucKhoeDoiSongRepository articleSucKhoeDoiSongRepository, IMediaSucKhoeDoiSongRepository mediaSucKhoeDoiSongRepository, MediaManagerSucKhoeDoiSong mediaManagerSucKhoeDoiSong, IDataSourceRepository dataSourceRepository, ICategorySucKhoeDoiSongRepository categorySucKhoeDoiSongRepository)
    {
        ...
        _mediaManagerSucKhoeDoiSong = mediaManagerSucKhoeDoiSong;
    ...
    public override async Task DoWorkAsync()
    {
        ...
        var articles = await _articleSucKhoeDoiSongRepository.GetListWithNavigationPropertiesAsync();
    ...
    private async Task PostToWPAsync(ArticleWithNavigationProperties articleNav)
    {
        ...
        if (articleNav.Categories != null)
        {
            ...
            if (result != null)
            {
                ...
                await _articleSucKhoeDoiSongRepository.UpdateAsync(articleNav.Article, true);
            }
        }
    }
    ...
    private async Task<List<MediaItem>> PostMediasAsync(Article article)
    {
        if (article is { Medias: { } })
        {
            ...
            var medias = await _mediaSucKhoeDoiSongRepository.GetListAsync(x =>mediaIds.Contains(x.Id));
            //pass the Wordpress REST API base address as string
            var client =await InitClient();
            ...
            if (medias != null)
            {
                ...
                foreach (var media in medias)
                {
                    try
                    {
                        var stream = await _mediaManagerSucKhoeDoiSong.GetFileStream(media.Name);
