...
namespace LC.Crawler.BackOffice.Medias
{
    ...
    {
        private readonly IMediaLongChauRepository _mediaRepository;
        private readonly IBlobContainer _blobContainer;

        public MediaManagerLongChau(IMediaLongChauRepository mediaRepository, IBlobContainer blobContainer)
        {
        ...
        private async Task SaveMedia(Media media)
        {
            ...
            if (fileBytes != null)
            {
                var fileName = $"{Enum.GetName(PageDataSource.LongChau)}{media.Id}{fileExtension}";
