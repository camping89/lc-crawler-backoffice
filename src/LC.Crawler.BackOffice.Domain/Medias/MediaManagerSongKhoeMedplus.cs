...
public class MediaManagerSongKhoeMedplus : DomainService
{
    private readonly IMediaSongKhoeMedplusRepository _mediaRepository;
    private readonly IBlobContainer _blobContainer;

    public MediaManagerSongKhoeMedplus(IMediaSongKhoeMedplusRepository mediaRepository, IBlobContainer blobContainer)
    {
    ...
    private async Task SaveMedia(Media media)
    {
        ...
        if (fileBytes != null)
        {
            var fileName = $"{Enum.GetName(PageDataSource.SongKhoeMedplus)}{media.Id}{fileExtension}";
