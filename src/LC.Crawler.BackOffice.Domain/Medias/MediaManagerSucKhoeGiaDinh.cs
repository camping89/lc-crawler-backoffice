...
public class MediaManagerSucKhoeGiaDinh : DomainService
{
    private readonly IMediaSucKhoeGiaDinhRepository _mediaRepository;
    private readonly IBlobContainer _blobContainer;

    public MediaManagerSucKhoeGiaDinh(IMediaSucKhoeGiaDinhRepository mediaRepository, IBlobContainer blobContainer)
    {
    ...
    private async Task SaveMedia(Media media)
    {
        ...
        if (fileBytes != null)
        {
            var fileName = $"{Enum.GetName(PageDataSource.SucKhoeGiaDinh)}{media.Id}{fileExtension}";
