...
public class WooManagerLongChau : DomainService
{
    ...
    public async Task DoSyncUpdateProduct()
    {
        ...
        var categories = (await _categoryLongChauRepository.GetListAsync(x => x.CategoryType == CategoryType.Ecom))
            .ToList();
                                ...
                                foreach (var wooCate in wooCategoriesFilter)
                                {
                                            ...
                                            StringComparison.InvariantCultureIgnoreCase))
                                    {
                                        ...
                                        if ((rootParent != null && category.Name.Contains(rootParent.name,
                                                StringComparison.InvariantCultureIgnoreCase)) || parentCate.parent == 0)
                                        {
                                            wooCategory = wooCate;
