...
public class WordpressManagerSongKhoeMedplus : DomainService
{
    ...
    public async Task DoUpdatePostAsync()
    {
        ...
        foreach (var post in posts)
        {
            ...
            try
            { 
                var article = await _articleSongKhoeMedplusRepository.GetAsync(x => x.ExternalId != null && x.ExternalId.Equals(post.Id.ToString()));
                var       articleNav    = await _articleSongKhoeMedplusRepository.GetWithNavigationPropertiesAsync(article.Id);
