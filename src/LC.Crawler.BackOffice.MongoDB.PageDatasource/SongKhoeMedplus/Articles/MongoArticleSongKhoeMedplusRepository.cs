...
namespace LC.Crawler.BackOffice.PageDatasource.SongKhoeMedplus.Articles;

public class MongoArticleSongKhoeMedplusRepository : MongoDbRepository<SongKhoeMedplusMongoDbContext, Article, Guid>,
    IArticleSongKhoeMedplusRepository
{
    public MongoArticleSongKhoeMedplusRepository(
        IMongoDbContextProvider<SongKhoeMedplusMongoDbContext> dbContextProvider) : base(dbContextProvider)
    {
    }

    public async Task<ArticleWithNavigationProperties> GetWithNavigationPropertiesAsync(Guid id,
        CancellationToken cancellationToken = default)
    {
        ...
        var media = await (await GetDbContextAsync(cancellationToken)).Medias.AsQueryable()
            .FirstOrDefaultAsync(e => e.Id == article.FeaturedMediaId, cancellationToken: cancellationToken);
        ...
        var categories = await (await GetDbContextAsync(cancellationToken)).Categories.AsQueryable()
            .Where(e => categoryIds.Contains(e.Id)).ToListAsync(cancellationToken: cancellationToken);
        var mediaIds = article.Medias.Select(x => x.MediaId).ToList();
        var medias = await (await GetDbContextAsync(cancellationToken)).Medias.AsQueryable()
            .Where(e => mediaIds.Contains(e.Id)).ToListAsync(cancellationToken: cancellationToken);
    ...
    public async Task<List<ArticleWithNavigationProperties>> GetListWithNavigationPropertiesAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var query = ApplyFilter((await GetMongoQueryableAsync(cancellationToken)), filterText, title, excerpt, content,
            createdAtMin, createdAtMax, author, tags, likeCountMin, likeCountMax, commentCountMin, commentCountMax,
            shareCountMin, shareCountMax, featuredMediaId, dataSourceId, categoryId, mediaId);
        var articles = await query.OrderBy(string.IsNullOrWhiteSpace(sorting)
                ? ArticleConsts.GetDefaultSorting(false)
                : sorting.Split('.').Last())
        ...
        Guid? mediaId = null)
    {
        return query
            .WhereIf(!string.IsNullOrWhiteSpace(filterText),
                e => e.Title.Contains(filterText) || e.Excerpt.Contains(filterText) || e.Content.Contains(filterText) ||
                     e.Author.Contains(filterText) || e.Tags.Contains(filterText))
            ...
            .WhereIf(featuredMediaId != null && featuredMediaId != Guid.Empty,
                e => e.FeaturedMediaId == featuredMediaId)
            .WhereIf(dataSourceId != null && dataSourceId != Guid.Empty, e => e.DataSourceId == dataSourceId)
            .WhereIf(categoryId != null && categoryId != Guid.Empty,
                e => e.Categories.Any(x => x.CategoryId == categoryId))
