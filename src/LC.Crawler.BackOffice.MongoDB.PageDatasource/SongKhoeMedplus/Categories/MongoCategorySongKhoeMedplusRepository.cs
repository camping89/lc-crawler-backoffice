...
namespace LC.Crawler.BackOffice.PageDatasource.SongKhoeMedplus.Categories;

public class MongoCategorySongKhoeMedplusRepository : MongoDbRepository<SongKhoeMedplusMongoDbContext, Category, Guid>,
    ICategorySongKhoeMedplusRepository
{
    public MongoCategorySongKhoeMedplusRepository(IMongoDbContextProvider<SongKhoeMedplusMongoDbContext> dbContextProvider) : base(dbContextProvider)
    {
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var category1 = await (await GetDbContextAsync(cancellationToken)).Categories.AsQueryable()
            .FirstOrDefaultAsync(e => e.Id == category.ParentCategoryId, cancellationToken: cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        var query = ApplyFilter((await GetMongoQueryableAsync(cancellationToken)), filterText, name, slug, description,
            parentCategoryId);
        var categories = await query.OrderBy(string.IsNullOrWhiteSpace(sorting)
                ? CategoryConsts.GetDefaultSorting(false)
                : sorting.Split('.').Last())
        ...
        Guid? parentCategoryId = null)
    {
        return query
            .WhereIf(!string.IsNullOrWhiteSpace(filterText),
                e => e.Name.Contains(filterText) || e.Slug.Contains(filterText) || e.Description.Contains(filterText))
            .WhereIf(!string.IsNullOrWhiteSpace(name), e => e.Name.Contains(name))
            .WhereIf(!string.IsNullOrWhiteSpace(slug), e => e.Slug.Contains(slug))
            .WhereIf(!string.IsNullOrWhiteSpace(description), e => e.Description.Contains(description))
            .WhereIf(parentCategoryId != null && parentCategoryId != Guid.Empty,
                e => e.ParentCategoryId == parentCategoryId);
    }
}